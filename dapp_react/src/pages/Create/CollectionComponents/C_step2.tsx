...
import {
  transactWithApproveCollection,
  transactWithMintCollection,
...
function C_step2() {
  ...
  const { collection, setCollection, resetCollection, account, signer } =
    useContext(GlobalContext) as GlobalContextType;
  const { files, setFiles } = useOutletContext<ContextType>();
  ...
  const getRemainedTimestamp = (inputDate: string) => {
  ...
  const handleSubmisstion = async () => {
    try {
      ...
      const collectionFile = collection.preReleaseJsonData.file;
      if (!collectionFile) return;
      const imageIpfsHash = await getImageIpfsHash(collectionFile);
      ...
      const tempIpfsHash = await pinJsonToIPFS(
        imageIpfsHash,
        metaData,
        jsonData
      );
      ...
      const mintResult = await transactWithMintCollection(
        signer,
        nftNameList,
        fileNameList,
        collectionIpfsHash,
      ...
      const getCollectionResult = await MintContract.methods
        .getCollectionData(account, tempIpfsHash)
        .call();
      console.log("getCollectionResult: ", getCollectionResult);

      const updateMetadataResult = await P_AddNftIdOnCollection(
        tempIpfsHash,
        getCollectionResult.ids
      );
      if (!updateMetadataResult.ok) return;

      // const collectionIds = getCollectionResult.ids.map(id => parseInt(id));
      const collectionIds = getCollectionResult.ids;
      // await
      ...
      const approveResult = await transactWithApproveCollection(
        signer,
        collectionIds
      );
      if (!approveResult.status) return null;

      const onsaleResult = await C_setOnsaleNfts(
        signer,
        collectionIds,
        collection.perPrice
      );
  ...
  const onChangePerPrice = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const onChangeStartAtData = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (inputDate === todayStr) {
      Swal.fire(
        "Air drop",
        "Collection 발행 뒤 10분 후 부터 에어드랍이 가능합니다"
      );
    } else {
  ...
  const onChangePreReleaseImage = async (
  ...
  return (
    <Container>
      ...
      <InputText
        type="number"
        onChange={onChangePerPrice}
        placeholder="ex) 0.1"
        value={collection.perPrice}
      />
      <InputLabel>Air drop 시작 날짜 및 시간</InputLabel>
      <InputText
        type="date"
        onChange={onChangeStartAtData}
        ...
        <PreviewWrap>
          <ContentWrap>
            <div style={{ marginBottom: "10px" }}>
              컬렉션의 각 NFT에는 최종 자산을 업로드하고 <br />
              공개할 때까지 사전 공개 미디어가 표시됩니다
            </div>
            <div
              style={{ marginBottom: "10px", color: "blue", cursor: "pointer" }}
            >
              더 알아보기
            </div>
